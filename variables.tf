...
variable "vpc_config" {
  description = "This contains the VPC configuration.  More specifically, the required cidr_block and the vpc name"
  type = object({
    cidr_block = string
    name       = string
  })
...
variable "subnet_config" {
  ...
  az         : The availabilty zone where to deploy the subnet
